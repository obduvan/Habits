...
class HabitRepository(private val habitsDao: com.example.data.room.HabitDao) : IHabitRepository {
    ...
    private suspend fun updateLocalDatabase(habit: HabitModel) {
        ...
        if (existingHabit != null) {
            if (existingHabit.date < habit.date) {
                ...
                habitsDao.update(com.example.data.room.HabitEntity.fromModel(habit))
            }
        } else {
            Log.i(HabitRepository::class.java.name, "${habit.name} new habit.")
            habitsDao.insert(com.example.data.room.HabitEntity.fromModel(habit))
        }
    }
    ...
    override suspend fun deleteHabit(habit: HabitModel): ApiResponse<Unit> {
        return try {
            ...
            habitsDao.delete(com.example.data.room.HabitEntity.fromModel(habit))
            ApiResponse.Success(data = Unit)
        } catch (e: RuntimeException) {
            handleErrors(e)
        }
